...
int main()
{
	...
	for (it = 0; it < NT; it++)
	{
		...
		// fp_snpx = fopen("vx_G.dat", "ab+");
		// fp_snpz = fopen("vz_G.dat", "ab+");
		// if ((it % dsnap) == 0)
		// {
		...
		// 		for (iz = 0; iz < NZ; iz++)
		// 		{
		// 			float utemp = (float)(vx_x[iz][ix] + vx_z[iz][ix]);
		// 			float wtemp = (float)(vz_x[iz][ix] + vz_z[iz][ix]);
		// 			fwrite(&utemp, sizeof(float), 1, fp_snpx);
		// 			fwrite(&wtemp, sizeof(float), 1, fp_snpz);
		// 		}
		// 	}
		// }
		// fclose(fp_snpx);
